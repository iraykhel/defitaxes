...
class Vault:
    ...
    def deposit(self,what, symbol,amount):
    ...
    def withdraw(self, what,symbol,amount, running_rates, coingecko_rates, transaction,tridx,fee_amount_per_transaction,fee_rate):
        ...
        if self.holdings[what] >= amount:
            ...
            while key_idx < len(holding_keys):
                usd_amt = amount*rate
        ...
        if amount > 0:
            ...
            incomes.append({'timestamp':timestamp,'text':'Income upon closing a vault', 'amount':amount*rate, 'hash':transaction['hash']})
...
class Loan:
    ...
    def repay(self, what,symbol,amount, running_rates, coingecko_rates, transaction,fee_amount_per_transaction,fee_rate):
        if what not in self.loaned:
            self.symbols[what] = symbol
            self.loaned[what] = 0
        ...
        if amount > 0:
            ...
            interest_payments.append({'timestamp': timestamp, 'text': 'Interest on a loan', 'amount': amount * rate, 'hash': transaction['hash']})
    ...
    def process_transactions(self,transactions_js):
        ...
        for tidx, transaction in enumerate(transactions_js):
            ...
            for transfer in transfers:
                ...
                if treatment == 'income':
                    self.incomes.append({'timestamp':timestamp,'text':'Yield farming or similar income', 'amount':amount*rate, 'hash':hash})
                    self.ca_transactions.append(CA_transaction(timestamp, what, symbol, amount, rate, txid,tridx, fee_amount_per_transaction, fee_rate))
                ...
                if treatment in ['borrow','repay']:
                    ...
                    if not outbound:
                        ...
                        print(transfer)
                        interest_payments = loan.repay(what,symbol,amount,running_rates,self.coingecko_rates,transaction,fee_amount_per_transaction,fee_rate)
                ...
                if treatment in ['deposit', 'withdraw']:
                    ...
                    vault = vaults[vaddr]
                    if outbound:
                        vault.deposit(what,symbol,amount)
                    else:
                        print(transfer)
                        v_trades, v_incomes, close = vault.withdraw(what,symbol,amount,running_rates,self.coingecko_rates,transaction,tridx,fee_amount_per_transaction,fee_rate)
    ...
    def matchup(self):
        ...
        for idx, ca_trans in enumerate(self.ca_transactions):
            ...
            if amount > 0:
                ...
                rate = ca_trans.rate
                while amount > 0 and amount * rate > 0.01:
                    try:
                        CA_in = q[0]
                    except:
                    ...
                    if CA_in.amount > amount:
                        ...
                        CA_line = {'symbol':symbol,'what':what,'amount':amount,'in_ts':CA_in.timestamp,'out_ts':ca_trans.timestamp,
                                   'basis':basis,'sale':amount*rate,'txid':txid,'tridx':tridx}
                        ...
                        CA_line = {'symbol': symbol, 'what': what, 'amount': CA_in.amount, 'in_ts': CA_in.timestamp, 'out_ts': ca_trans.timestamp,
                                   'basis': basis, 'sale': CA_in.amount * rate, 'txid': txid, 'tridx': tridx}
